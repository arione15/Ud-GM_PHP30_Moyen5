...
1. On filtre les livres selon certaines propriétés => donc on doit comparer (`if`) ces propriétés des livres parcourus (`foreach`) avec les valeurs de ces propriétés qui se trouvent  en paramètre de la fonction. Ces paramètres de la fonction qui vont être, lors de l'appel, rempli par `$_GET` ou `$_POST`.
